...
import {
  ArrowRightIcon,
  UserGroupIcon, // Pour les agents/équipes
  MegaphoneIcon, // Pour la distribution de flyers / promotion
  ShoppingCartIcon, // Pour la revente de produits/services
  BuildingStorefrontIcon, // Pour les foires/salons/liquidation en magasin
  ChartBarIcon, // Pour l'analyse de ventes / performance
  CurrencyEuroIcon, // Pour l'optimisation des ventes / liquidation
  GlobeAltIcon, // Pour la portée/expansion commerciale
  HandRaisedIcon, // Pour le support personnalisé
...
// Adapté pour les services d'agents commerciaux
const AgentCommercialServices = [
  {
    name: 'Distribution de Flyers Ciblée',
    description:
      'Nos agents distribuent vos flyers et supports publicitaires dans les zones stratégiques pour atteindre efficacement votre public cible.',
    icon: MegaphoneIcon, // Icône pour la distribution/publicité
    color: 'from-emerald-500 to-green-600',
    stat: 'Visibilité maximale garantie',
  },
  {
    name: 'Revente de Produits & Services',
    description:
      'Confiez-nous la commercialisation de vos offres. Nos agents qualifiés maximisent vos ventes et développent votre clientèle.',
    icon: ShoppingCartIcon, // Icône pour la revente/commerce
    color: 'from-blue-500 to-indigo-600',
    stat: 'Augmentation de votre chiffre d\'affaires',
  },
  {
    name: 'Agents Commerciaux en Extra',
    description:
      'Bénéficiez de l\'expertise de nos agents professionnels pour vos salons, foires, ou toute opération ponctuelle nécessitant une force de vente additionnelle.',
    icon: UserGroupIcon, // Icône pour les équipes/agents
    color: 'from-purple-500 to-pink-600',
    stat: 'Flexibilité et performance assurées',
  },
  {
    name: 'Liquidation & Déstockage',
    description:
      'Nous vous aidons à écouler rapidement vos stocks excédentaires ou vos invendus lors d\'événements spécifiques ou en magasin.',
    icon: CurrencyEuroIcon, // Icône pour la liquidation/argent
    color: 'from-cyan-500 to-blue-600',
    stat: 'Optimisation de vos stocks',
  },
  {
    name: 'Prospection & Développement Commercial',
    description:
      'Nos agents identifient de nouvelles opportunités et prospectent activement pour étendre votre portée commerciale et conquérir de nouveaux marchés.',
    icon: GlobeAltIcon, // Icône pour l'expansion/prospection
    color: 'from-orange-500 to-red-600',
    stat: 'Expansion rapide de votre marché',
  },
  {
    name: 'Formation & Coaching Vente',
    description:
      'Nous formons vos équipes ou nos agents aux meilleures techniques de vente, assurant une approche professionnelle et des résultats concrets.',
    icon: HandRaisedIcon, // Icône pour le coaching/support
    color: 'from-teal-500 to-cyan-600',
    stat: 'Performance commerciale améliorée',
  },
];

// Adapté pour les services d'agents commerciaux (catégories)
const features = [
  {
    icon: UserGroupIcon,
    title: 'Agents Dévoués',
  },
  {
    icon: MegaphoneIcon,
...

  {
    ...
    title: 'Liquidation Stocks',
  },
];

export default function AgentCommercial() { // Renommé le composant en "AgentCommercial"
    ...
    <div className="w-full min-h-screen flex-shrink-0 snap-center flex flex-col justify-center items-center text-black bg-transparent relative overflow-hidden">
      ...
      <div className="relative z-10 w-full max-w-[90rem] mx-auto px-4 sm:px-6 lg:px-8 xl:px-12 2xl:px-16">
        ...
        >
          ...
          >
            <TextPressure
              text="SERVICES D'AGENTS COMMERCIAUX" // NOUVEAU TITRE
              flex={true}
              alpha={false}
              stroke={false}
              width={true}
          ...
          >
            <ShinyText
              text="Mettez une équipe commerciale dédiée à votre service pour booster vos ventes, distribuer vos flyers, revendre vos produits ou liquider vos stocks en toute efficacité." // Nouvelle description
              disabled={false}
              speed={3}
          ...
          >
            <div className="bg-white/80 backdrop-blur-sm rounded-2xl p-4 sm:p-6 shadow-lg border border-white/20">
              <div className="text-2xl sm:text-3xl font-bold text-indigo-600">Agents</div> {/* Stat adaptée */}
              <div className="text-xs sm:text-sm text-gray-600">Professionnels</div> {/* Description stat adaptée */}
            </div>
            <div className="bg-white/80 backdrop-blur-sm rounded-2xl p-4 sm:p-6 shadow-lg border border-white/20">
              <div className="text-2xl sm:text-3xl font-bold text-emerald-600">+30%</div> {/* Stat adaptée */}
              <div className="text-xs sm:text-sm text-gray-600">Ventes Boostées</div> {/* Description stat adaptée */}
            </div>
            <div className="bg-white/80 backdrop-blur-sm rounded-2xl p-4 sm:p-6 shadow-lg border border-white/20">
              <div className="text-2xl sm:text-3xl font-bold text-purple-600">Rapidité</div> {/* Stat adaptée */}
              <div className="text-xs sm:text-sm text-gray-600">d'Intervention</div> {/* Description stat adaptée */}
            </div>
          </motion.div>
        </motion.div>

        {/* Sales Agent Categories (features) - Centré avec largeur adaptative */}
        <motion.div
          initial={{ opacity: 0, y: 30 }}
          whileInView={{ opacity: 1, y: 0 }}
          transition={{ duration: 0.8 }}
        ...
        >
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 sm:gap-8 w-full max-w-sm sm:max-w-xl lg:max-w-4xl">
        ...
        {/* Services Grid (details) - Grid adaptatif centré */}
        <motion.div
        ...
        >
          <div className="text-center mb-12 sm:mb-16">
            <h3 className="text-2xl sm:text-3xl font-bold text-gray-900 mb-3 sm:mb-4">Pourquoi faire appel à nos agents commerciaux ?</h3> {/* Nouveau titre */}
            <p className="text-base sm:text-lg text-gray-600 max-w-xs sm:max-w-xl md:max-w-2xl mx-auto px-4">
              Découvrez les bénéfices d'une collaboration avec une équipe d'agents professionnels dédiée à votre succès commercial.
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-6 sm:gap-8 max-w-xs sm:max-w-none mx-auto justify-items-center">
            {AgentCommercialServices.map((service, i) => ( // Utilise AgentCommercialServices
              <motion.div
                key={service.name}
        ...
        >
          <div className="bg-gradient-to-r from-indigo-50 to-purple-50 rounded-3xl p-8 sm:p-12 border border-indigo-100 w-full max-w-xs sm:max-w-2xl md:max-w-3xl lg:max-w-4xl">
            <h3 className="text-2xl sm:text-3xl font-bold text-gray-900 mb-3 sm:mb-4">
              Prêt à booster vos ventes et votre visibilité ?
            </h3> {/* Nouveau titre */}
            <p className="text-base sm:text-lg text-gray-600 mb-6 sm:mb-8 max-w-xs sm:max-w-xl md:max-w-2xl mx-auto">
              Contactez-nous pour une stratégie commerciale personnalisée et découvrez l'impact de nos agents professionnels.
            </p> {/* Nouvelle description */}
